var _contentNavBarElement;
var _contentClassElements;
//var _contentClass;
var _document;

function populateContentSidebar(contentClassElements, navBarElement, Document) {
	_contentNavBarElement = navBarElement;
...
function refreshContentSidebar(){
	...
	var contentCounter;
	//Remove previous content shortcut links from the content navbar element
	removeAllChildElements(_contentNavBarElement);
...
//If this element has attributes, proceed
	if(element.hasAttributes())
	{
		...
		if(hasIdAttribute)
		{
			createShortcutLinkForElement(idAttributeValue, hasHiddenAttribute);
		}			
	}
}
...
function createShortcutLinkForElement(elementID, hiddenStatus){
    ...
    //Append the new element to the side navbar    
